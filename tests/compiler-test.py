...
description = "Compile each test-case  with both GCC and CustomCompiler, and then compare the results.",
epilog      = ""
)
...
argparser.add_argument('-w','--wrapper',metavar='PATH',
                       help='Invoke the Custom compiler through the shell script at PATH. (default: use `compiler-wrapper.sh` from the same directory as compiler-test.py)')
...
orig_cwd=os.getcwd()
if "compiler-test-output" in orig_cwd:
    ## we are invoked from within a preexisting DEST subtree => let's use it in place
    DEST="./"
    while os.path.basename(os.path.realpath('.')) != 'compiler-test-output':
        os.chdir('..')
        DEST="../"+DEST
    os.chdir(orig_cwd)
    ...
    if not os.path.isdir('compiler-test-output'):
        os.mkdir('compiler-test-output')
    DEST='compiler-test-output'
...
if args.wrapper:
    ...
    wrapper=os.path.dirname(os.path.realpath(__file__))+"/compiler-wrapper.sh"
...
for inputfilename in inputfilenames:
    ...
    if ('compiler-test-output' in os.path.realpath(inputfilename)
        ...
        ## ../somedir/subdir/file.c becomes ./compiler-test-output/somedir-subdir-file/input.c
...
for jobname in jobs:
    ...
    ## PADAWAN compiler
    compilerstatus=command(wrapper+" asm-compiler.s input.c", "compiler-compile.txt")
    
    if gccstatus != 0 and compilerstatus != 0:
        ## padawan correctly rejects invalid program -> test-case ok
        print("TEST OK")
        continue
    elif gccstatus != 0 and compilerstatus == 0:
        ...
        if args.verbose:
            dumpfile("compiler-compile.txt")
        continue
    else:
        ## padawan accepts to compile valid program -> let's link it
        ldstatus=command("gcc -o exe-compiler asm-compiler.s", "compiler-link.txt")
        if ldstatus:
            print("TEST FAIL (your compiler produces incorrect assembly)")
            if args.verbose:
                dumpfile("compiler-link.txt")
            continue

    ## both compilers  did produce an  executable, so now we  run both
    ## these executables and compare the results.
        
    execompilerstatus=command("./exe-compiler","compiler-execute.txt")
    if open("gcc-execute.txt").read() != open("compiler-execute.txt").read() :
        print("TEST FAIL (different results at execution)")
        if args.verbose:
            print("GCC:")
            dumpfile("gcc-execute.txt")
            print("you:")
            dumpfile("compiler-execute.txt")
        continue
